...
const navItems: NavItem[] = [
  ...
  { path: '/users', label: '使用者管理', icon: Users, adminOnly: true },
];

function getCurrentUser() {
  try {
    const raw = localStorage.getItem('chiayou_user');
...
export default function Layout({ children }: LayoutProps) {
  ...
  const avatarChar = currentUser?.full_name?.charAt(0) ?? '?';
